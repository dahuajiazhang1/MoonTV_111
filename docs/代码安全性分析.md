...
**说明**: 所有敏感环境变量都在 `.gitignore` 中，不会被推送到 GitHub
...
- ⚠️ 密码是明文存储在 Redis 中
- ✅ 但 Redis 连接凭据在环境变量中，不会泄露
- ✅ 代码中没有暴露任何用户密码

**建议**: 考虑使用 bcrypt 等哈希算法加密密码（可选）
...
> 所有支付信息存储在 Upstash 数据库，代码中只有读取逻辑
...
- ⚠️ 如果 Upstash 被攻破，密码会泄露
- ✅ 但 Upstash 连接凭据安全，风险可控
...
这些必须在 Netlify 配置，**不要**提交到 GitHub：
