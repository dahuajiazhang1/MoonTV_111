...
async function testGetPaymentSettings() {
  ...
  if (result.success) {
    ...
    // 验证数据
    if (
      result.data.alipay_qr_code === testData.paymentSettings.alipay_qr_code
    ) {
...
async function testUserSubscription() {
  ...
  const result = await apiRequest(
    `/api/user/subscription?username=${testData.testUser.username}`
  );
