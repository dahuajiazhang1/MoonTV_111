...
export default function OrderManager() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return (
          <span className='flex items-center gap-1 text-green-600 bg-green-50 px-2 py-0.5 rounded text-xs'>
      ...
      case 'pending':
        return (
          <span className='flex items-center gap-1 text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded text-xs'>
            <Clock size={12} /> 待审核
          </span>
        );
      default:
              ...
              <tr key={order.order_no}>
                ...
                <td className='px-4 py-3'>
                  <div className='font-medium text-blue-600'>
                    {order.plan_name || `套餐ID: ${order.related_id}`}
                  </div>
                  ...
                  {order.payment_status === 'approved' &&
                    ...
                    order.plan_duration && (
                      <div className='text-xs text-gray-400 mt-0.5'>
                        到期:{' '}
                        {new Date(
                          new Date(order.verified_at).getTime() +
                            order.plan_duration * 24 * 60 * 60 * 1000
                        ).toLocaleDateString()}
                      </div>
                    )}
                </td>
